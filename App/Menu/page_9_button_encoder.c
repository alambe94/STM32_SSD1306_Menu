...
static void Show_Encoder_Screen0()
{
    ...
    ssd1306_WriteString(" BTN", Font_11x18, White);
...
static void Show_Encoder_Screen2()
{
    ssd1306_Fill(Black);
    ssd1306_SetCursor(0, 0);
    ssd1306_WriteString(" ENC", Font_11x18, White);
...
static uint8_t Run_Encoder_Screen1(Menu_Event_t *event)
{
    ...
    ssd1306_WriteString("*BTN*", Font_11x18, White);
    ssd1306_SetCursor(0, 20);
    snprintf(buff, sizeof(buff), "  %d", event->Enter_Button_Clicks);
    ssd1306_WriteString(buff, Font_11x18, White);
    ssd1306_SetCursor(0, 40);
    ssd1306_WriteString("<--Long Press", Font_7x10, White);
    ssd1306_UpdateScreen();

    if (event->Enter_Button_Clicks == 255)
    {
        xreturn = 0;
    }
...
static uint8_t Run_Encoder_Screen2(Menu_Event_t *event)
{
...
void Menu_Add_Encoder_Page()
{
    ...
    Encoder_Screen_List[2].Show_Page_Item = Show_Encoder_Screen2;
    Encoder_Screen_List[2].Page_Item_Callback = Run_Encoder_Screen2;
